// Integration tests for cargo-fumi.
...
/// Run the cargo-fumi executable and return its output.
fn cargo_fumi(args: &[&str]) -> (String, String) {
    let mut bin_dir = env::current_exe().unwrap();
    bin_dir.pop(); // chop off test exe name
    if bin_dir.ends_with("deps") {
        bin_dir.pop();
    }
    let cmd = bin_dir.join(format!("cargo-fumi{}", env::consts::EXE_SUFFIX));

    // Ensure cargo-fumi runs the rustfumi binary from the local target dir.
...
macro_rules! assert_that {
    ($args:expr, $check:ident $check_args:tt) => {
        let (stdout, stderr) = cargo_fumi($args);
        if !stdout.$check$check_args {
            panic!(
                "Output not expected for cargo-fumi {:?}\n\
                 expected: {}{}\n\
                 actual stdout:\n{}\n\
                 actual stderr:\n{}",
                $args,
...
#[test]
fn version() {
    assert_that!(&["--version"], starts_with("rustfumi "));
    assert_that!(&["--version"], starts_with("rustfumi "));
    assert_that!(&["--", "-V"], starts_with("rustfumi "));
    assert_that!(&["--", "--version"], starts_with("rustfumi "));
}
...
fn cargo_fmt_emits_error_on_line_overflow_true() {
    ...
    let args = [
        "--check",
        "--manifest-path",
        "tests/cargo-fumi/source/issue_3164/Cargo.toml",
        "--",
        "--config",
        "error_on_line_overflow=true",
    ];

    let (_stdout, stderr) = cargo_fumi(&args);
